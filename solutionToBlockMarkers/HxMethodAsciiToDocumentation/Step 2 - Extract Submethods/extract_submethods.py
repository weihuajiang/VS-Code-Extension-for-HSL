...
def main():
    ...
    output_file = r'c:\Users\admin\Desktop\submethods_output.csv'
    
    print(f"Parsing {input_file}...")
    submethods = parse_submethods(input_file)
    
    print(f"Found {len(submethods)} submethods")
    
    # Debug: Show first few methods
    for i, method in enumerate(submethods[:5]):
        print(f"\n{i+1}. {method['name']}")
        print(f"   Description length: {len(method['description'])}")
        print(f"   Variables: {len(method['variables'])}")
        if method['variables']:
            for v in method['variables'][:3]:
                print(f"     - {v['name']}: {v['description'][:50] if v['description'] else '(no desc)'}")
