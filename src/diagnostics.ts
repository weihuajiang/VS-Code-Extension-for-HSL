...
/**
 * Returns true if the document is an auto-generated sub-method library
 * header (.hs_) produced by the Hamilton Method Editor.  These files are
 * machine-generated and should never be hand-edited, so running
 * diagnostics on them only produces false positives.
 *
 * Detection: file extension is .hs_ AND one of the first two non-empty
 * lines is the marker comment "// this ALWAYS GENERATED file contains
 * the sub-method library header".
 */
function isAutoGeneratedSubMethodHeader(document: vscode.TextDocument): boolean {
  if (!document.fileName.endsWith('.hs_')) {
    return false;
  }
  // Check the first two lines for the marker comment.
  const linesToCheck = Math.min(document.lineCount, 2);
  for (let i = 0; i < linesToCheck; i++) {
    if (document.lineAt(i).text.includes('ALWAYS GENERATED file contains the sub-method library header')) {
      return true;
    }
  }
  return false;
...
async function refreshDiagnostics(
  ...
  // Skip diagnostics entirely for auto-generated sub-method library
  // headers -- they contain design-time stubs that trigger false
  // positives for missing-declaration and variable-placement rules.
  if (isAutoGeneratedSubMethodHeader(document)) {
